...
//
// The AES lookup tables. Rcon and sbox are used for key expansion, sbox and isbox for
// encryption and decryption. The tables are loaded into the flash memory on the Arduino to
// save RAM using the PROGMEM modifier. Normal const array is used on other platforms.
// References: 
...
#else // Anything but the Arduino platform

// AES inverse SBox lookup table
const byte_ard isbox[] = {
    0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb, 
    0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb, 
    0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42, 0xfa, 0xc3, 0x4e, 
    0x08, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25, 
 ...
 */
void KeyExpansion(const byte_ard *key, byte_ard *ckeys) 
{
  memcpy(ckeys,key,16); // Copy the first key
	...
	for(int i=16; i<176; i+=4)
	{
        ...
        if (i % 16 == 0)  // Dividable by 16, the first key
		{
			...
			ckeys[i] = ckeys[i-16] ^ getSboxByte(ckeys[i-3]) ^ getRconByte(r++);
			ckeys[i+1] = ckeys[i-15] ^ getSboxByte(ckeys[i-2]);
			ckeys[i+2] = ckeys[i-14] ^ getSboxByte(ckeys[i-1]);
			ckeys[i+3] = ckeys[i-13] ^ getSboxByte(ckeys[i-4]);
		}
		else
		{
...
void SubAndShift(void *pText)
{
	...
	state(pState,0,0) = getSboxByte(state(pState,0,0));
	state(pState,0,1) = getSboxByte(state(pState,0,1));
	state(pState,0,2) = getSboxByte(state(pState,0,2));
	state(pState,0,3) = getSboxByte(state(pState,0,3));

	// Shift and sbox the second row
	temp=state(pState,1,0);
	state(pState,1,0)=getSboxByte(state(pState,1,1));
	state(pState,1,1)=getSboxByte(state(pState,1,2));
	state(pState,1,2)=getSboxByte(state(pState,1,3));
	state(pState,1,3)=getSboxByte(temp);
	// Shift and sbox the third row
	temp = state(pState,2,0);
	state(pState,2,0)=getSboxByte(state(pState,2,2));
	state(pState,2,2)=getSboxByte(temp);
	temp = state(pState,2,1); 
	state(pState,2,1)=getSboxByte(state(pState,2,3));
	state(pState,2,3)=getSboxByte(temp);
	// Shift and sbox the fourth row
	temp = state(pState,3,3);
	state(pState,3,3) = getSboxByte(state(pState,3,2));
	state(pState,3,2) = getSboxByte(state(pState,3,1));
	state(pState,3,1) = getSboxByte(state(pState,3,0));
	state(pState,3,0) = getSboxByte(temp);
...
void InvSubAndShift(void *pText)
{
  ...
  state(pState,0,0) = getISboxByte(state(pState,0,0));
  state(pState,0,1) = getISboxByte(state(pState,0,1));
  state(pState,0,2) = getISboxByte(state(pState,0,2));
  state(pState,0,3) = getISboxByte(state(pState,0,3));
  
  // Second row is shifted one byte to the left
  temp = state(pState,1,3);
  state(pState,1,3) = getISboxByte(state(pState,1,2));
  state(pState,1,2) = getISboxByte(state(pState,1,1));
  state(pState,1,1) = getISboxByte(state(pState,1,0));
  state(pState,1,0) = getISboxByte(temp);

  // Third row is shifted two bytes to the left
  temp = state(pState,2,2);
  state(pState,2,2) = getISboxByte(state(pState,2,0));
  state(pState,2,0) = getISboxByte(temp);
  temp = state(pState,2,1);
  state(pState,2,1) = getISboxByte(state(pState,2,3));
  state(pState,2,3) = getISboxByte(temp);

  // The fourth row is shifted three bytes to the left
  temp = state(pState,3,0);
  state(pState,3,0) = getISboxByte(state(pState,3,1));
  state(pState,3,1) = getISboxByte(state(pState,3,2));
  state(pState,3,2) = getISboxByte(state(pState,3,3));
  state(pState,3,3) = getISboxByte(temp);
...
// FIXME: Move padding logic into this function
// CBCEncrypt()
 ...
 *  getRconByte
 *
 *  Getter for the Rcon lookup table. 
 *  This table is stored in PROGMEM on the Arduino to save RAM.
 */
byte_ard getRconByte(int index)
{
#ifdef _ARDUINO_DUEMILANOVE
  return pgm_read_byte_near(Rcon+index);
#else
  return Rcon[index];
#endif
}
 ...
 *  getSboxByte
 *
 *  Getter for the sbox lookup table. 
 *  This table is stored in PROGMEM on the Arduino to save RAM.
 */
byte_ard getSboxByte(int index)
{
#ifdef _ARDUINO_DUEMILANOVE
  return pgm_read_byte_near(sbox+index);
#else
  return sbox[index];
#endif
}
 ...
 *  getISboxByte
 *
 *  Getter for the isbox lookup table. 
 *  This table is stored in PROGMEM on the Arduino to save RAM.
 */
byte_ard getISboxByte(int index)
{
#ifdef _ARDUINO_DUEMILANOVE
  return pgm_read_byte_near(isbox+index);
#else
  return isbox[index];
#endif
}
