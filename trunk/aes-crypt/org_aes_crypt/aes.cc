...
//
// Here are some output options. Only enable the verbose output for debugging. Especially useful
// to trace the intermediary options for debugging and verification. The intermediary output is set
// up to be easily comparable with the intermediary steps of the test from Appendix B of the
// FIPS-197 document. Of course, this is less true of the t-transform version, which mashes
// together most of the steps!
//
//#define verbose
//#define verbose_debug
//#define text_mode_output
//#define verbose_errors
//#define debug_print_tbox_construction
 ...
 *  printBytes
 *
 *  Prints a buffer of the specified length. The output is by default chopped at every 16th byte.
 *  This is used for final output only in "production" mode.
 */
#if defined(verbose) || defined(verbose_debug) || defined(text_mode_output)
 ...
 *  printState
 *
 *  Print the AES state in a readable manner.
 *  This is used only for debugging and verification of individual steps.
 */
#ifdef verbose_debug
#define getState(a,i,j)  (a[j] >> (8*i) & 0xFF)
void printState(unsigned int pState[4])
{
	for(int i=0;i<4;++i)
	{
		printf("[%d,%d]: %.2x  ", i, 0, getState(pState,i,0) );
		printf("[%d,%d]: %.2x  ", i, 1, getState(pState,i,1) );
		printf("[%d,%d]: %.2x  ", i, 2, getState(pState,i,2) );
		printf("[%d,%d]: %.2x\n", i, 3, getState(pState,i,3) );
	}
	printf("\n");
}
#endif
...
inline void addRoundKey(void *pText, const unsigned int *pKeys, int round)
{
	...
	#ifdef verbose_debug
	printf("\nAdding round key at round #%d:\n", round);
	printBytes((unsigned char*)(pKeys+4*round),16);
	printf("\nState at round (#%d)\n", round);
	printState((unsigned int *)pText);
	#endif
}
...
inline void subAndShift(void *pText)
{
	...
	#ifdef verbose_debug
	printf("State after subAndShift:\n");
	printState((unsigned int *)pText);
	#endif 
}
#endif
...
inline void encryptBlock(void *pText, const unsigned int *pKeys)
{
	...
	printBytes((unsigned char *)pText,16);
	printf("\nState at beginning of encryption:\n");
	printState((unsigned int *)pText);
	#endif
...
/*
	The following is the temporary code used to generate the ttables defined at the top
    of the source.
...
int main(int argc, char *argv[])
{
	...
	#ifdef verbose_debug
	printf("\nKey schedule:\n");
	printBytes((unsigned char *)pKeys,16*11);
	printf("\n\n");
	#endif
