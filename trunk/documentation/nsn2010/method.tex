...
%
The original Needham-Schroeder was vulnerable to replay attacks \shortcite{denning1981}, which can be fixed by using nonces and/or timestamps \cite{needham1987}. We use primarily nonces in our protocols to counter replay attacks.

As discussed in Section~\ref{sec:authenc}, we use the \textit{Encrypt-then-MAC (EtM)} \shortcite{bellare2007} composition paradigm, in which the plaintext is first encrypted and then a MAC of the ciphertext appended. We use $\mathcal{T}_K$ in our protocol to indicate \textit{tagging} (MACing) of the message ciphertext with a shared symmetric secret key $K$. $\mathcal{E}_K$ and $\mathcal{D}_K$ is used for encryption (resp.\ decryption) with a shared symmetric secret key $K$.
...
If $T$ is accepted, then $A$ generates a random 128-bit long session key $K_{ST}$ and
...
The session key has now been delivered to $T$ and $S$. This key is only
...
%However, we maintain a soft state for intermediary protocol
