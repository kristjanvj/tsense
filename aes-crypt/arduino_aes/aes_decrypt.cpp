 ...
 * 
 * AES Decryption Implementation. FIPS-197, Section 5.3 Inverse cipher
 *
 * Intel 32/64 and Arduino platform.
 *
 */

#include "aes_decrypt.h"
// xtime macro used in the InvMixColumns transformation. Implements the left shift and 
// conditional XOR operaton described in FIPS-197 section 4.2.1.
// The twotimes and threetimes macros are based on the decription by Daemen and Rijmen.
#define xtime(a) (a<<1) ^ ((a & 0x80) ? 0x1b : 0x00)  
#define twotimes(a) (((a<<1) ^ (((a>>7) & 1) * 0x1b)) & 0xFF)
#define threetims(a) (a^twotimes(a))
...
// Access an element in the state (a linear char array)
...
// Rijandel Key Schdeule Rcon operation lookup table (Round Constant word arry)
// Borrowed from http://en.wikipedia.org/wiki/Rijndael_key_schedule
...
// The same dummy key (for testing) as in aes_crypt.cpp
byte_ard pKey[] = {
...
// KeyExpansion()
//
// The AES key expansion algorithm. (128 bit keys and 10 rounds)
// http://en.wikipedia.org/wiki/Rijndael_key_schedule
// This is practically the same code as in aes_crypt.cpp by KVJ
// key = the encryption key. keys = the derived expension keys. 

void KeyExpansion(const void *key, void *keys)
{
  memcpy(keys, key, 16); // The first 16 bytes of the expanded key is simply the encryption key
  
  #ifdef verbose_output
  Serial.println("Starting key expansion");
  Serial.print("Initial key: ");
  ...
  int r=1;     //The Rcon iterator
  for (int i=16; i<176; i+= 4)
  {
    // The SubWord and RotWord methods described in FIPS-197 are replaced here by their inline
    // equivalents. The Algorithm is also simplified by not supporting the longer key lengths. 
    // Several steps are combined to be able to compute keys in-place without temporary variables.

    if (i % 16 == 0) 
    {
      // Copy the four previous bytes with rotate. Apply the SBox to the four bytes of the key only
    ...
    {
      // Stright copy and rotate of the previous key bytes
...
// AddRoundKey()
// The round key is added to the sate by XOR
// (XOR-ing the key to the state)
void AddRoundKey(void *pText, const u_int32_ard *pKeys, int round)
{
  ...
  // Arduino-specific debug. (Borrowed from KVJ)
  #ifdef verbose_debug
...
//
// Inverted
// The first row isnt rotated and thus isboxed only. 
// The second row is rotated one byte to the left and isboxed
// The third row is rotated two byets to the left and isboxed
// The fourth row is rotated three bytes to the left and isboxed
...
// ntransforms -- normal transfor macro to help with the loop unrolling. Modified for decrypt
#define ntransform(text,keys,round) InvSubAndShift(text);AddRoundKey(text,keys,round);InvMixColumns(text);

// DecryptBlock()
// 
// Decrypt a single block, stored in the buffer 'text'. THe buffer MUST be 16 bytes in length!
